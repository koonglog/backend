...
*   **Dashboard Analytics**: 단지 내 온라인 센서 현황, 금일 발생 경고 횟수, 실시간 평균 소음 수치 등 핵심 지표를 계산하여 제공하는 통계 API 개발.
*   **Admin Management**: 생성된 중재 메시지의 상태(대기/승인/발송완료) 관리 및 이력 조회 기능.

## 🏗 System Architecture
1.  **IoT Simulator**: 실시간 소음 수치 발생 및 서버 전송.
2.  **FastAPI Server**: 데이터 수신 및 AI 로직(Rule-based Message Generator) 실행.
...
| `GET` | `/api/v1/noise-logs` | 전체 소음 데이터 이력 조회 |
| `GET` | `/api/v1/mediations` | AI가 생성한 중재 메시지 초안 목록 조회 |
| `PATCH`| `/api/v1/mediations/{id}` | 중재 메시지 상태 업데이트 (관리자 승인 등) |
